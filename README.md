...
Simulated Camera Scanning: Receiving drivers can instantly scan this QR code via an on-screen camera-view viewport, dynamically re-assigning the database's current_driver_id and writing a transfer event to the milestone ledger.
...
CREATE TABLE public.milestone_logs (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    parcel_id TEXT REFERENCES public.parcels(id) ON DELETE CASCADE NOT NULL,
...
ALTER TABLE public.milestone_logs DISABLE ROW LEVEL SECURITY;


⚙️ Environment Configurations (.env)

Create a .env file in your root folder. Vite requires environment variables to start with VITE_ to be correctly bundled into client files:

VITE_SUPABASE_URL=[https://lffblhlsrvvujjapwbjb.supabase.co](https://lffblhlsrvvujjapwbjb.supabase.co)
VITE_SUPABASE_ANON_KEY=your_actual_supabase_anon_public_key_string
...
Select "Add New Project" and import your cleardrop-mvp repository.

Open Environment Variables in the Vercel Setup Wizard and add:
...
React uses client-side routing. If you navigate to sub-routes (e.g., /driver or /track) on a mobile phone, Vercel will attempt to look up physical folders on its server and return a 404 NOT_FOUND error.
